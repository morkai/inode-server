...
module.exports = function setUpHttp(app, httpConfig)
{
  ...
  {
    if (req.url.startsWith(httpConfig.gsmUploadUrl))
    {
  ...
  function handleDevicesRequest(id, req, res, body)
  {
    ...
    const matches = req.url.match(/^\/devices\/([0-9]+|(?:[A-Fa-f0-9]{2}:?){5}:?[A-Fa-f0-9]{2})$/);
  ...
  {
    gsmReportMap[report.address] = restoreReportProperties(report);
   ...
   * @returns {Object}
   */
  function restoreReportProperties(obj)
  {
    Object.keys(obj).forEach(k =>
    {
      ...
      if (typeof v === 'object')
      {
        ...
        {
          obj[k] = new Buffer(v.data);
        }
        else
        {
          obj[k] = restoreReportProperties(v);
        }

        return;
      }

      if (typeof v === 'string' && /^[0-9]{4}.*?T.*?Z$/.test(v))
      {
